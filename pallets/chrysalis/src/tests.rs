...
use ark_ff::{BigInteger, PrimeField}; // Removed unused UniformRand, Zero
// Removed unused SNARK, Groth16, ProvingKey
// Removed unused CanonicalDeserialize, CanonicalSerialize
use ark_sponge::{poseidon::{PoseidonConfig, PoseidonSponge}, CryptographicSponge}; // Keep CryptographicSponge
// Removed unused thread_rng
// Removed unused ark_crypto_primitives
// Removed unused ark_relations, tracing
use sp_core::H256;
use std::str::FromStr;
use hex; // Keep hex
...
// Keep setup_poseidon_params if other tests need it (like calculate_zero_hashes)
// --- Helper Functions for Tests ---
...
// Keep calculate_zero_hashes if other tests need it
fn calculate_zero_hashes(params: &PoseidonConfig<Fr>, depth: usize, default_leaf: Fr) -> Vec<Fr> {
    let mut zero_hashes = Vec::with_capacity(depth + 1);
    let mut current_zero = default_leaf;
    zero_hashes.push(current_zero);
    for _i in 0..depth {
        current_zero = hash_fr(params, current_zero, current_zero);
        zero_hashes.push(current_zero);
    }
    zero_hashes
}
...
// Helper to convert Fr string back to H256
fn fr_str_to_h256(fr_str: &str) -> H256 {
    let fr = Fr::from_str(fr_str).expect("Failed to parse Fr string from JSON");
    let mut bytes = [0u8; 32];
    let fr_bytes_vec = fr.into_bigint().to_bytes_le();
    bytes[..fr_bytes_vec.len().min(32)].copy_from_slice(&fr_bytes_vec);
    H256::from(bytes)
}
// ***************************************
...
fn transfer_with_used_nullifier_should_fail() {
	new_test_ext().execute_with(|| {
		let used_nullifier = H256::from([5; 32]);
		Nullifiers::<Test>::insert(used_nullifier, ());
        // FIX: Ensure Error enum is accessible via Pallet::<Test>::Error
		assert_err!(
			Pallet::<Test>::transfer(
				RuntimeOrigin::signed(1), Vec::new(), H256::default(),
				[used_nullifier, H256::from([6; 32])],
				[H256::default(), H256::default()],
			),
			Error::<Test>::NullifierAlreadyUsed // Use full path to Error variant
		);
	});
}
...
fn end_to_end_transfer_should_succeed() {
	...
	new_test_ext().execute_with(|| {
        ...
        let pub_inputs_json_str = fs::read_to_string("src/testdata/test_pub_inputs.json")
            .expect("Failed to read test_pub_inputs.json. Did you run generate_test_data and copy the files?");
        let pub_inputs_data: PublicInputsData = serde_json::from_str(&pub_inputs_json_str)
        ...
        let nullifiers_h256 = [
            fr_str_to_h256(&pub_inputs_data.nullifiers[0]),
            fr_str_to_h256(&pub_inputs_data.nullifiers[1]),
        ];
        let output_commitments_h256 = [
