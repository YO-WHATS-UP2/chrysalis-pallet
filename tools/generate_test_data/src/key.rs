...
// Import your circuit definition
use chrysalis_circuit::{Note, PrivateTransferCircuit}; 

use std::{fs::File, io::Write, path::PathBuf, str::FromStr};
use serde::{Deserialize, Serialize};
use hex;

// --- PASTE YOUR PROVING KEY BYTES HERE ---
pub const PROVING_KEY_BYTES: &[u8] = &[
    // Note: This constant is unused in SETUP MODE, but keep it here for easy replacement
];
// ----------------------------------------

// --- Helper: Poseidon Params (Must exist locally with full constant arrays) ---
// Note: You must ensure the actual function body contains the full 117 constants and MDS matrix.
fn setup_poseidon_params() -> PoseidonConfig<Fr> {
    let full_rounds = 8;
...
// --- Helper function to WRITE bytes as a Rust array ---
fn write_bytes_as_rust_array(writer: &mut impl Write, bytes: &Vec<u8>, const_name: &str) -> std::io::Result<()> {
    writeln!(writer, "\n// --- {} ({} bytes) ---", const_name, bytes.len())?;
    write!(writer, "pub const {}: &[u8] = &[", const_name)?;
...
fn main() -> std::io::Result<()> {
    ...
    println!("Setting up Poseidon parameters...");
    let poseidon_params = setup_poseidon_params(); // This should now successfully load the constants from your local function

    println!("Creating dummy circuit for setup (TreeDepth = 2)...");
    // Create a dummy circuit with the CORRECT Day 9 structure
    let dummy_circuit = PrivateTransferCircuit {
        merkle_root: Fr::default(),
        nullifiers: [Fr::default(); 2],
        output_commitments: [Fr::default(); 2],
        // NEW PUBLIC INPUT: Must be here for setup
        withdrawal_amount: Fr::default(), 
        // --- Private Witness ---
        input_notes: [Note { secret: Fr::default(), value: Fr::default() }; 2],
        // *** CRITICAL: Set array sizes to 2 ***
        input_merkle_paths: [[Fr::default(); 2]; 2],
        input_merkle_directions: [[false; 2]; 2],
        // ***
        input_private_keys: [Fr::default(); 2],
        output_notes: [Note { secret: Fr::default(), value: Fr::default() }; 2],
        // NEW WITNESS: The change note
        change_note: Note { secret: Fr::default(), value: Fr::default() }, 
        
        poseidon_params,
    };

    println!("Running Groth16::setup (this will take some time)...");
    let (pk, vk) = Groth16::<Bls12_381>::setup(dummy_circuit, &mut rng)
        .expect("Groth16 setup failed");
    
    println!("Setup complete! Serializing keys...");

    let mut pk_bytes = Vec::new();
    pk.serialize_uncompressed(&mut pk_bytes).expect("Failed to serialize PK");

    let mut vk_bytes = Vec::new();
    vk.serialize_uncompressed(&mut vk_bytes).expect("Failed to serialize VK");

    // --- Create and write to the output file ---
    let out_file_path = PathBuf::from("keys_output.txt");
    let mut file = File::create(&out_file_path)?;
    println!("Saving keys to {}...", out_file_path.display());

    writeln!(&mut file, "--- COPY AND PASTE THESE KEYS ---")?;
    
    write_bytes_as_rust_array(&mut file, &vk_bytes, "VERIFYING_KEY")?;
    write_bytes_as_rust_array(&mut file, &pk_bytes, "PROVING_KEY_BYTES")?;

    writeln!(&mut file, "---")?;
    println!("Successfully saved keys to keys_output.txt");
    
    // Print to console for convenience
    write_bytes_as_rust_array(&mut std::io::stdout(), &vk_bytes, "VERIFYING_KEY")?;
    write_bytes_as_rust_array(&mut std::io::stdout(), &pk_bytes, "PROVING_KEY_BYTES")?;
